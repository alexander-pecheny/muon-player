...
swiftc -O scripts/muon-send.swift MuonPlayerMac/TransferSender.swift \
  ...
  MuonPlayer/Models/Track.swift MuonPlayer/Scanner/FolderArt.swift -o /tmp/muon-send
/tmp/muon-send --list
...
the artwork of everything it did not look at. `FolderArt.scan` is a full walk of the roots,
unlike the audio pass beside it: dropping a `cover.jpg` into a folder changes no track's
mtime, and the folder index cannot say which folders once held one. So it runs only on a
pass that listed something, and an idle scan is still a stat per folder.
