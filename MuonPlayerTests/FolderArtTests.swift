...
struct FolderArtTests {
    ...
    @Test("The scan picks the best image in each folder")
    func picksBestPerFolder() throws {
        ...
        let art = FolderArt.scan(roots: [root])
        #expect(art.count == 1)
        #expect(art[album.path]?.hasSuffix("cover.jpg") == true)
    }
